 ...
 e.fn.mdSlider = function(r) {
  ...
  function D() {
   ...
   if (r.showBullet != false) {
    ...
    if (r.showThumb) {
     ...
     for (var u = 0; u < f; u++) {
      var c = o[u].data("thumb"),
          ...
          v = o[u].data("thumb-alt");
      if (c) {
       var E;
       if (h == "image") E = e("<img />").attr("src", c).attr("alt", o[u].data("thumb-alt")).css({
        top: -(9 + l) + "px",
        left: -(a / 2 - 2) + "px",
        opacity: 0
       });
       else E = e("<span></span>").attr("style", c).css({
        top: -(9 + l) + "px",
        left: -(a / 2 - 2) + "px",
        opacity: 0
       });
       e("div.md-bullet:eq(" + u + ")", g).append(E).append('<div class="md-thumb-arrow" style="opacity: 0"></div>')
      }
     }
    ...
    var S = e('<div class="md-thumb"><div class="md-thumb-container"><div class="md-thumb-items"></div></div></div>').appendTo(p);
    w = e(".md-thumb-items", S);
    for (var u = 0; u < f; u++) {
     var c = o[u].data("thumb"),
         h = o[u].data("thumb-type"),
         v = o[u].data("thumb-alt");
     if (c) {
      var x = e('<a class="md-thumb-item" />').attr("rel", u);
      if (h == "image") x.append(e("<img />").attr("src", c).attr("alt", o[u].data("thumb-alt")));
      else x.append(e("<span />").attr("style", c).css("display", "inline-block"));
      w.append(x)
     }
    }
  ...
  function ot() {
   var t = 1;
   //TODO Need to improve checking browser version
   browser = get_browser_info()
   if (parseInt(browser.version, 10) < 9) t = 6;
   if (p.width() < r.width) {
    e(".md-objects", s).css({
